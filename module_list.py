...
def dice_loss(_pred, target, smooth=1e-5):
    eps = 1e-7
    # convert target to onehot
    targ_onehot = torch.eye(_pred.shape[1])[target].permute(0, 3, 1, 2).float().cuda()
    # convert logits to probs
    pred = activation(_pred)
    # sum over HW
    inter = (pred * targ_onehot).sum(axis=[-1, -2])
    union = (pred + targ_onehot).sum(axis=[-1, -2])
    # mean over C
    loss = 1. - (2. * inter / (union + eps)).mean(axis=-1)
    return loss.mean()

def giouloss(_pred, target, smooth=1e-5):
    """
        Args:
        ...
        """
    eps = 1e-7
    inter, union = _box_inter_union(_pred, target)
    iou = inter / union
