...
# model.load_state_dict(torch.load('model_weights/dice_loss_epoch60_aichallenge_label5_semi_classmix_reco_0.pth', map_location=torch.device('cuda')))
# model.eval()

total_epoch = 200
optimizer = optim.SGD(model.parameters(), lr=0.001, weight_decay=args.weight_decay, momentum=0.9, nesterov=True)
...
for index in range(total_epoch):
    ...
    for i in tqdm(range(train_epoch)):
        ...
        loss = sup_loss + unsup_loss + reco_loss*10+cls_loss
        loss.backward()
        optimizer.step()
        ema.update(model)
    ...
    if args.apply_reco:
        torch.save(ema.model.state_dict(), 'model_weights/dice_loss_epoch{}_{}_label{}_semi_{}_reco_{}.pth'.format(index,args.dataset, args.num_labels, args.apply_aug, args.seed))
    else:
        torch.save(ema.model.state_dict(), 'model_weights/dice_loss_epoch{}{}_label{}_semi_{}_{}.pth'.format(index,args.dataset, args.num_labels, args.apply_aug, args.seed))
