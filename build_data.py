...
def transform(image, label, logits=None, crop_size=(512, 512), scale_size=(0.8, 1.0), augmentation=True):
    ...
    #print(raw_w,resized_size)
    if ori_resized_size[1] < resized_size[1] or ori_resized_size[0] < resized_size[0]:
        #print("cropping")
...
def one_sample_transform(image, label, logits=None, crop_size=(448, 448), scale_size=(0.8, 1.0), augmentation=True):
    # Random rescale image
    raw_w, raw_h = image.size
    # scale_ratio = random.uniform(scale_size[0], scale_size[1])
    #
...
def no_resize_transform(image, label, logits=None, crop_size=(512, 512), scale_size=(0.8, 1.0), augmentation=True):
    ...
    # image = transforms_f.resize(image, crop_size, Image.BILINEAR)
    # label = transforms_f.resize(label, crop_size, Image.NEAREST)
...
def aichallenge_class_map(mask):
    # source: https://github.com/mcordts/cityscapesScripts/blob/master/cityscapesscripts/helpers/labels.py
    # mask_map = np.zeros_like(mask)
    mask_map = mask
    ...
    # mask_map[np.isin(mask, [3])] = 2
    # mask_map[np.isin(mask, [4])] = 3
    # mask_map[np.isin(mask, [13])] = 4
    # mask_map[np.isin(mask, [17])] = 5
    # mask_map[np.isin(mask, [19])] = 6
    # mask_map[np.isin(mask, [20])] = 7
...
def unlabeled_aichallenge_class_map(mask):
    # source: https://github.com/mcordts/cityscapesScripts/blob/master/cityscapesscripts/helpers/labels.py
    mask_map = np.zeros_like(mask)[...,0]
    # mask_map[np.isin(mask_map, [0])] = 0
    return mask_map
...
class BuildDataset(Dataset):
    def __init__(self, root, dataset, idx_list, crop_size=(512, 512), scale_size=(0.5, 2.0),
                 augmentation=True, train=True, apply_partial=None, partial_seed=None,islabel=False):
        ...
        self.class_map = {'container_truck':0, 'forklift':1, 'reach_stacker':2, 'ship':3}
...
class BuildDataLoader:
    def __init__(self, dataset, num_labels, crop_size=[321, 321],batch_size=1):
        self.dataset = dataset
        if dataset == 'pascal':
...
def color_map_with_id(mask, colormap):
    color_mask = np.zeros([mask.shape[0], mask.shape[1], 3])
    u_id=[]
    for i in np.unique(mask):
        color_mask[mask == i] = colormap[i]
    ...
    return np.uint8(color_mask),u_id
